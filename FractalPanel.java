	...
	public void drawKochLine( Graphics2D g2, int level, KochLine line ) {
		...
		if (level == 0) {
			...
			if (line.isKochLineValid()) {
				...
				int aX = line.getLineA().getStart().getX();
				int aY = line.getLineA().getStart().getY();
				int dX = line.getLineD().getEnd().getX();
				int dY = line.getLineD().getEnd().getY();
				
				g2.drawLine(aX,aY,dX,dY);
			}
		}
